<template>
  <div class="app-container mkTextbook">
    <el-card>
      <basics-search
        ref="searchs"
        class="searchs"
      ...
      >
        <template slot="type" slot-scope="scope">
          <dict-tag :options="dict.type.saas_book_type" :value="scope.row.type"/>
        </template>
...
import { mkTextbook, updateMaterialStatus, batchDelMaterial  } from "@/api/runSchool/share";
...
export default {
  ...
  methods: {
    init(){
      this.$nextTick(()=>{
        const { saas_book_type, saas_professional_group_level } = this.dict.type;
    ...
    tableAction(type, row) {
      ...
      switch (type) {
        ...
        case "view":
          // 查看
          this.$router.push({
            path: "/runSchool/share/addmkTextbook",
            query: { type, id },
          });
          break;
        case "remove":
          this.$modal.confirm("确定删除该活动信息么？").then(() => {
            batchDelMaterial({ ids: id }).then(() => {
        ...
        case "notPass":
          ...
          this.$confirm("确定提交么？", "提示", { type: "warning" }).then(
            () => {
              updateMaterialStatus([{ cid, id, status: statusMap[type] }]).then(
                () => {
    ...
    handleCheck(status) {
      ...
      this.$confirm("确定批量操作么？", "提示", { type: "warning" }).then(
        () => {
          updateMaterialStatus(payload).then(() => {
            this.$message.success("操作成功~");
            this.getList();
          });
        }
