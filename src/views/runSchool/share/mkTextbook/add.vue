...
<script>
import { mkTextbookForm, companyNameData, allForm } from "../industry/form";
import { remoteMethod } from "@/utils/commonExampleMethods"
import { mkTextbook, addOrUpdateMaterial, getMaterialInfo } from "@/api/runSchool/share";
...
export default {
  ...
  dicts: ["saas_book_type", "saas_professional_group_level"],
  computed: {
    ...mapState(["user"]),
  },
  mounted() {
  ...
  methods: {
    init() {
      let initForm = mkTextbookForm(this.querySearch).concat(allForm);
      // 指定位置添加多个元素
    ...
    handleEdit(id) {
      this.loading = true;
      this.$nextTick(() => {
        getMaterialInfo({ id }).then(({ payload }) => {
          this.slotForm = {
    ...
    setOptionsData() {
      this.$nextTick(() => {
        const { saas_book_type, saas_professional_group_level} =
          this.dict.type;
        this.$refs?.form.set({
          type: "options",
          data: {
    ...
    async querySearch(name, cb) {
      let results = [];
      if (name) {
        const res = await mkTextbook({ name });
        results = res.rows || [];
        cb(results);
      } else {
        cb([]);
