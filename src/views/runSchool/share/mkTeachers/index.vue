<template>
  <div class="app-container mkTeachers">
    <el-card>
      <basics-search
        ref="searchs"
        class="searchs"
      ...
      >
        <template slot="type" slot-scope="scope">
          <dict-tag :options="dict.type.saas_teacher_type" :value="scope.row.type"/>
        </template>
      </basics-table>
    </el-card>
  </div>
...
import { mkTeachers, updateTeacherStatus, batchDelTeacher } from "@/api/runSchool/share";
...
export default {
  ...
  methods: {
    init(){
      this.$nextTick(()=>{
        const { saas_teacher_type } = this.dict.type;
    ...
    tableAction(type, row) {
      ...
      switch (type) {
        ...
        case "view":
          // 查看
          this.$router.push({
            path: "/runSchool/share/addmkTeachers",
            query: { type, id },
          });
          break;
        case "remove":
          this.$modal.confirm("确定删除该活动信息么？").then(() => {
            batchDelTeacher({ ids: id }).then(() => {
        ...
        case "notPass":
          ...
          this.$confirm("确定提交么？", "提示", { type: "warning" }).then(
            () => {
              updateTeacherStatus([{ cid, id, status: statusMap[type] }]).then(
                () => {
    ...
    handleCheck(status) {
      ...
      this.$confirm("确定批量操作么？", "提示", { type: "warning" }).then(
        () => {
          updateTeacherStatus(payload).then(() => {
            this.$message.success("操作成功~");
            this.getList();
          });
        }
