...
export default {
  ...
  methods: {
    ...
    getList() {
      ...
      const listFn = activeType === '1' ? getCoverageList : getExchageList
      listFn(params)
        .then((res) => {
          if (res.code === 200) {
            this.list = res.rows || [];
    ...
    tableAction(type, row) {
      ...
      switch (type) {
        ...
        case "view":
          this.$router.push({
            path: "/runSchool/addAffect",
            query: { activeType, type, id },
          });
          break;
        case "del":
          const delFn =
            activeType === "1" ? batchDelCoverage : batchDelExchange;
        ...
        case "reject":
          ...
          const fn =
            activeType === "1" ? updateCoverageStatus : updateExchangeStatus;
    ...
    handleDelete() {
      ...
      const delFn = activeType === "1" ? batchDelCoverage : batchDelExchange;
    ...
    handleExport() {
      ...
      const exportApi =
        this.activeType === "1"
          ? "/v1/influence/exportCoverage"
          : "/v1/influence/exportExchange";
      this.download(
        exportApi,
        {},
        `办学影响_${new Date().getTime()}.xlsx`
      ).then(() => loading.close());
    },
    // 导入
    uploadBefore(file) {
      const importFn =
        this.activeType === "1" ? importCoverage : importExchange;
